...
[DONE] sparsity_analysis.jl – Detect sparse structures   !!! Needs better ADTypes integration

[DONE] stiffness_analysis.jl – Eigenvalue-based stiffness detection   !!! Needs better ADTypes integration

timescale_analysis.jl – Fast/slow decomposition

coupling_analysis.jl – Reaction–diffusion coupling metrics

condition_analysis.jl – Condition number and Jacobian sensitivity
...
backend_interface.jl – Core backend interface

linsolve_interface.jl – Linear solver backend control

AD_interface.jl – Abstract AD backend interface

sparse_forwarddiff.jl – ForwardDiff with sparsity support

enzyme_backend.jl – Enzyme.jl implementation

finite_difference.jl – Fallback finite difference backend

symbolic_backend.jl – Symbolics.jl backend

hybrid_backend.jl – Backend switching logic

backend_selector.jl – Heuristic and performance-based backend choice

Features:

Unified interface for AD and linear algebra

Automatic fallback (symbolic → AD → FD)

Backend selection based on structure + performance

Sparsity-aware and memory-efficient evaluation
