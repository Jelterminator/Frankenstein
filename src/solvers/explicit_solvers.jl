# explicit_solvers.jl - Explicit Runge-Kutta and multistep methods
...
Strategy for selecting explicit methods suitable for non-stiff problems.
"""
...
function create_explicit_algorithms()
    return [
        ...
        AlgorithmRecommendation(
            ...
            stiffness_range=(NON_STIFF, NON_STIFF),
        ...
        AlgorithmRecommendation(
            ...
            stiffness_range=(NON_STIFF, NON_STIFF),
            system_size_range=(SMALL_SYSTEM, SMALL_SYSTEM),
...
function get_explicit_recommendations(analysis::SystemAnalysis;
    ...
    # Only recommend explicit methods for non-stiff problems
    stiffness = classify_stiffness(analysis)
    if stiffness != NON_STIFF
    ...
    applicable = filter(alg -> is_applicable(alg, analysis, target_rtol), algorithms)
